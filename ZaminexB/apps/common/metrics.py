...
def build_neighborhood_price_per_sqm_map(properties: list[Property] | None = None) -> dict[str, float]:
...
def price_deviation_index(
    ...
    neighborhood_avg: dict[str, float] | None = None,
) -> float | None:
    pps = price_per_sqm(effective_sale_price(property_obj), property_obj.area)
    if pps is None or not property_obj.neighborhood:
        return None
    if neighborhood_avg is None:
        neighborhood_avg = build_neighborhood_price_per_sqm_map([property_obj])
    avg = neighborhood_avg.get(property_obj.neighborhood)
    if not avg:
        return None
...
def property_market_metrics(
    ...
    dom = _days_between(property_obj.created_at) if property_obj.created_at else None
    return {
        ...
        "priceDeviationIndex": price_deviation_index(property_obj, neighborhood_avg),
...
def consultant_avg_deal_probability(user) -> float | None:
    agg = (
        FollowUp.objects.filter(consultant=user, is_archived=False)
        .exclude(probability__isnull=True)
        .aggregate(avg=Avg("probability"))
    )
    avg = agg.get("avg")
    if avg is None:
        return None
...
def consultant_performance_metrics(profile) -> dict[str, Any]:
    ...
    return {
        ...
        "avgDealProbability": consultant_avg_deal_probability(user),
    }
