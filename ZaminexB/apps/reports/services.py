...
def compute_property_report(prop: Property, *, filters: dict | None = None) -> dict[str, Any]:
    ...
    overdue_tasks = [
        t
        for t in tasks
        if t.status != Task.Status.COMPLETED and t.due_date and t.due_date < today
    ]
    ...
    # ---- 3. Avg_Deal_Probability (per follow-up type) -----------------------
    probs_by_type: dict[str, list[int]] = {}
    for f_ in followups:
        if f_.probability is None:
            continue
        probs_by_type.setdefault(
            f_.get_follow_up_type_display() or f_.follow_up_type, []
        ).append(int(f_.probability))
    avg_prob_chart = []
    overall_probs: list[int] = []
    for label, values in sorted(probs_by_type.items()):
        if not values:
            continue
        avg_prob_chart.append(
            {
                "label": label,
        ...
        overall_probs.extend(values)
    avg_deal_probability = (
        round(sum(overall_probs) / len(overall_probs), 2) if overall_probs else None
    )
    if avg_deal_probability is None:
        warnings.append("هیچ پیگیری با احتمال معامله برای این ملک ثبت نشده است.")
    ...
    if prop.pk:
        comparables_qs = comparables_qs.exclude(pk=prop.pk)
    comp_count = comparables_qs.count()
    if comp_count >= 2:
        avg_map = _build_map(list(comparables_qs))
        neighborhood_avg = avg_map.get(prop.neighborhood)
        comparables_source = "همسایه‌های هم‌نوع"
    else:
        # Fallback: all properties in the neighborhood regardless of deal_type
        comparables_qs = Property.active_objects.exclude(area=0).filter(
    ...
    kpis = {
        ...
        "avgDealProbability": avg_deal_probability,
        "pricePerSqm": pps,
    ...
    charts = {
        ...
        "avgDealProbabilityByType": avg_prob_chart,
        "priceMap": map_points,
...
CSV_TRANSLATIONS = {
    ...
    "avgDealProbability": "میانگین احتمال معامله",
