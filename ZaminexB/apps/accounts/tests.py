...
class ConsultantFullDeletionTests(TestCase):
    ...
    def test_delete_removes_user_and_all_owned_data(self):
        url = reverse("accounts:consultant-detail", args=[self.profile.id])
        res = self.client_api.delete(url)
        self.assertEqual(res.status_code, status.HTTP_204_NO_CONTENT)

        self.assertFalse(ConsultantProfile.objects.filter(pk=self.profile.pk).exists())
        ...
        self.assertFalse(FollowUp.objects.filter(pk=self.followup.pk).exists())
    ...
    def test_delete_keeps_other_consultants_data(self):
        ...
        self.assertIsNone(other_task.assigned_to)  # unassigned, not deleted
