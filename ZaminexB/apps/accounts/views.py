...
class ConsultantProfileViewSet(viewsets.ModelViewSet):
    ...
    def perform_destroy(self, instance):
        """Delete the consultant *completely*.

        The default DRF destroy only removes the ConsultantProfile row and
        leaves the linked accounts.User behind in the database (so the
        account could still log in). Here we clean up every relation that
        references the consultant's user account — including the PROTECT
        relations (follow-ups, created tasks/listings, properties) — inside
        a single transaction, and finally delete the user itself, which
        cascades to the profile, notifications, etc.
        """
        ...
        with transaction.atomic():
            # Re-lock the row inside the transaction to avoid races.
            instance = ConsultantProfile.objects.select_for_update().get(pk=instance.pk)
            user = instance.user

            # Follow-ups always belong to a consultant (FK PROTECT) -> remove them.
            FollowUp.objects.filter(consultant=user).delete()

            # Tasks: unassign the ones assigned to this consultant,
            # delete the ones they created (FK PROTECT on created_by).
            Task.objects.filter(assigned_to=user).update(assigned_to=None)
            Task.objects.filter(created_by=user).delete()

            # Listings: unassign assigned ones, delete created ones (PROTECT).
            Listing.objects.filter(assigned_to=user).update(assigned_to=None)
            Listing.objects.filter(created_by=user).delete()

            # Properties owned by this consultant (FK PROTECT). Deleting a
            # property cascades its listings/images and nulls related
            # task/follow-up property links.
            Property.objects.filter(consultant=user).delete()

            # Remove the uploaded avatar file from media storage.
            if instance.profile_image:
                instance.profile_image.delete(save=False)

            # Finally delete the user account; the ConsultantProfile goes
            # with it via CASCADE (as do notifications, sessions, etc.).
            user.delete()
