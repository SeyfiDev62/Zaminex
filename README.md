...
You don't need this to run the project. Bundles are already built in `ZaminexB/static/frontend/assets/`.

If you edit files in `ZaminexF/src/`:
...
After build, you will have new files in `ZaminexF/dist/assets/` like:
```
index-XXXXX.js
index-YYYYY.css
```
...
```bash
# Delete old assets
rm ../ZaminexB/static/frontend/assets/*

# Copy new ones
cp -r dist/assets/* ../ZaminexB/static/frontend/assets/

# Open ../ZaminexB/templates/base.html and update 2 lines to match new file names:
# <link rel="stylesheet" href="{% static 'frontend/assets/index-YYYYY.css' %}">
# <script type="module" src="{% static 'frontend/assets/index-XXXXX.js' %}"></script>
```

Example: if dist has `index-ABC123.js` and `index-DEF456.css`, base.html should point to those exact names.
